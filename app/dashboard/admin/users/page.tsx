...
function UsersManagementPage({ user }: { user: any }) {
  ...
  const fetchAllUserSubscriptions = async () => {
    try {
      ...
      for (const userItem of users) {
        try {
          const response = await fetch(`https://workspaceapi-b81x.onrender.com/push/subscriptions/${userItem._id}`, {
            headers: { Authorization: `Bearer ${token}` }
          })
  ...
  const sendNotificationToUser = async (userId: string) => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`https://workspaceapi-b81x.onrender.com/push/send-to-user/${userId}`, {
        method: "POST",
        headers: {
  ...
  const updateUserRole = async (userId: string, newRole: string) => {
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`https://workspaceapi-b81x.onrender.com/users/${userId}`, {
        method: "PUT",
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`

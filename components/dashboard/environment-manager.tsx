...
export function EnvironmentManager() {
  ...
  const handleCreateEnvironment = async () => {
    ...
    try {
      ...
      const response = await fetch("https://workspaceapi-b81x.onrender.com/entornos/crear-entornos", {
        method: "POST",
        headers: { 
  ...
  const toggleEnvironmentStatus = async (environmentId: string) => {
    try {
      ...
      // Si estamos offline, el interceptor de fetch guardará la petición en IndexedDB
      // Si estamos online, la petición se ejecutará normalmente
      const response = await fetch(`https://workspaceapi-b81x.onrender.com/entornos/cambiar-estado/${environmentId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
  ...
  const deleteEnvironment = async (environmentId: string) => {
    ...
    try {
      // Si estamos offline, el interceptor de fetch guardará la petición en IndexedDB
      // Si estamos online, la petición se ejecutará normalmente
      const response = await fetch(`https://workspaceapi-b81x.onrender.com/entornos/eliminar/${environmentId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  const handleEditEnvironment = async () => {
    ...
    try {
      // Si estamos offline, el interceptor de fetch guardará la petición en IndexedDB
      // Si estamos online, la petición se ejecutará normalmente
      const response = await fetch(`https://workspaceapi-b81x.onrender.com/entornos/editar/${editEnvironmentId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
